...
\section{Infinite-dimensional SDEs}
...
where:
\begin{itemize}
    \item $H$ is an infinite-dimensional Hilbert space,
    \item $A : H \to H$ is typically an unbounded linear operator,
    \item $W_t^Q$ is a $Q$-Wiener process, a generalization of the finite-dimensional Wiener process with covariance operator $Q$ reflecting spatial correlations inherent in infinite-dimensional spaces.
\end{itemize}
